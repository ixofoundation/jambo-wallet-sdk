...
class Opera {
	#PUBKEYS: { [chainId: string]: { pubkeyByteArray: Uint8Array; bech32Address: string } } = {};
	...
	async getOfflineSigner(chainId: string): Promise<OfflineDirectSigner | null> {
		...
		const getAccounts = async (): Promise<readonly AccountData[]> => {
			const pubVars = this.#PUBKEYS[chainId];
			if (!pubVars) return [];
			return [{ address: pubVars.bech32Address, algo: ALGO_SECP as Algo, pubkey: pubVars.pubkeyByteArray }];
		};
		const offlineSigner: OfflineDirectSigner = { getAccounts, signDirect: this.signDirect };
		return offlineSigner;
	}

	async signDirect(chainId: string, signDoc: SignDoc): Promise<DirectSignResponse> {
		const pubVars = this.#PUBKEYS[chainId];
